...
class App extends Sprite {
	...
	private stage_onKeyDown = (event:KeyboardEvent):void => {
		...
		switch (event.keyCode) {
			...
			case Keyboard.R: this.Logo.rotation += 10; break; // add rotation
			case Keyboard.Q: this.Logo.rotation -= 10; break;
			case Keyboard.COMMAND && Keyboard.EQUAL: this.Logo.scaleX += .25; this.Logo.scaleY += .25; break;
			case Keyboard.COMMAND && Keyboard.MINUS: this.Logo.scaleX -= .25; this.Logo.scaleY -= .25; break;
	...
	private stage_onKeyUp = (event:KeyboardEvent):void => {
		...
		switch (event.keyCode) {
			...
			case Keyboard.SPACE: this.Logo.x = 100; this.Logo.y = 100; break; // add reset button with space
			default: preventDefault = false;
...
AssetLibrary.loadFromManifest (manifest).onComplete ((library) => {
	...
	var stage = new Stage (1500, 800, 0xFFFFFF, App);
